...
yes = [pd.read_csv(f'outputs/noclouds_off{i}.csv', parse_dates = ['datetime']) for i in range(3)]

#resampling del yes, usando el valor promedio de 2 a 32
yes_sample = [i[(i.datetime.dt.minute == 2) | (i.datetime.dt.minute == 32)] 
              for i in yes]

yes_merged = [i.merge(brew, on = 'datetime', how = 'right')
              for i in yes_sample]


# for i in range(3):
#     y = yes_merged[i]
#     y['h'] = y.datetime.dt.hour
#     plt.figure()
#     plt.hist(y.h)
# plt.show()
...
for k in range(3):
    ...
    for n,i in enumerate(instruments):
        ...
        print(f'{i}   {1/model.slope}x  {-1*(model.intercept/model.slope)}')
        ...
        axss[0,n].tick_params(axis='both', labelsize=12)        
        axss[1,n].tick_params(axis='both', labelsize=12)
    fig.supxlabel('Brewer UVB [W/m2]', y = 0.015, fontsize = 12)
    fig.suptitle(f'Correlation - Offset {k} w/o clouds', fontsize = 16)
    #     print(i)
    #     print(model.table)
    fig.savefig(f'figures/correlation_brewer_woclouds_off{k}.png', dpi = 300)
#plt.show()
...
for k in range(3):
    ...
    fig.subplots_adjust(wspace=0.05, hspace=0.03, left = 0.05, right = 0.97)
    for n,i in enumerate(instruments):
        yes_merged_ = yes[k].dropna()
        x = yes_merged_.uvb
        y = yes_merged_[f'{i}_uvb']
        model = iterative_linear(x, y, 3, 2)
        #print(i, r2, m, b)
        axss[0,n].scatter(x, y, c = 'k', s = 2)
        axss[0,n].set_title(i, fontsize = 13)
        axss[0,n].plot(x, model.slope*x + model.intercept, c = 'r')
        axss[0,n].annotate(f'R2 = {round(model.r2,3)}', (0.1, 0.85), xycoords = 'axes fraction', fontsize = 13)
        axss[0,n].annotate(f'{round(model.slope,2)}x + {round(model.intercept,2)}', (0.1, 0.9), xycoords = 'axes fraction', fontsize = 13)
        axss[0,0].set_ylabel('Instrument UVB [W/m2]', fontsize = 12)
        axss[1,n].scatter(x, model.slope*x + model.intercept - y, s = 2, c = 'k')
        axss[1,n].axhline(0, c = 'k', lw = 1, alpha = .3)
        axss[1,0].set_ylabel('Residual [W/m2]', fontsize = 12)
        axss[0,n].tick_params(axis='both', labelsize=12)
        axss[1,n].tick_params(axis='both', labelsize=12)
    fig.supxlabel('TUV UVB [W/m2]', y = 0.015, fontsize = 12)
    fig.suptitle(f'Correlation - Offset {k} w/o clouds', fontsize = 16)
    #     print(i)
    #     print(model.table)
    fig.savefig(f'figures/correlation_tuv_woclouds_off{k}.png', dpi = 300)
